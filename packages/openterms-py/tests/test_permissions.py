...
import openterms
from openterms.permissions.cache import TermsCache
from openterms.permissions.client import OpenTermsClient, _normalise_domain, _sha256
from openterms.permissions.models import (
    CacheEntry,
    CheckResult,
    DiscoveryResult,
    PermissionReceipt,
...
def test_receipt_contains_correct_fields() -> None:
    ...
    rec = client.permission_receipt("example.com", "api_access", "allow")
    assert isinstance(rec, PermissionReceipt)
...
def test_receipt_empty_hash_when_not_cached() -> None:
    client = OpenTermsClient(cache=TermsCache())
    rec = client.permission_receipt("unreachable.com", "api_access", "not_specified")
    assert rec.openterms_hash == ""


def test_receipt_to_dict() -> None:
    client = OpenTermsClient(cache=TermsCache())
    rec = client.permission_receipt("example.com", "scrape_data", "deny")
    d = rec.to_dict()
    assert set(d.keys()) == {
...
def test_module_level_receipt() -> None:
    openterms.clear_cache()
    rec = openterms.permission_receipt("example.com", "api_access", "allow")
    assert rec.domain == "example.com"
    assert rec.decision == "allow"
    openterms.clear_cache()
