...
All public functions (fetch, check, discover, permission_receipt) are
module-level convenience wrappers around the OpenTermsClient class. The
module ships a single shared client instance; call configure() to adjust
its settings.
"""
...
from .models import CacheEntry, CheckResult, DiscoveryResult, PermissionReceipt
...
_DEFAULT_USER_AGENT: str = "openterms-py/1.0"
...
class OpenTermsClient:
    ...
    def __init__(
        ...
        # Use explicit identity check — TermsCache.__len__ makes empty caches
        # falsy, so `cache or default` would silently discard a valid empty cache.
        self._cache: TermsCache = cache if cache is not None else get_default_cache()
    ...
    def fetch(self, domain: str) -> dict[str, Any] | None:
        """
    ...
    def check(self, domain: str, action: str) -> CheckResult:
        """
        ...
        For a signed audit trail of permission checks, call
        :meth:`openterms.IngestClient.emit_receipt` with
        ``action_type="permission_check"`` and the check result in
        ``action_context``. The 0.4.x ``receipt=True`` parameter was removed
        in 1.0.0 — see CHANGELOG.
        """
        domain = _normalise_domain(domain)
    ...
    def discover(self, domain: str) -> DiscoveryResult | None:
        """
    ...
    def permission_receipt(
        ...
        """
        Generate a minimal local permission-check receipt.
...
def fetch(domain: str) -> dict[str, Any] | None:
    """
...
def check(domain: str, action: str) -> CheckResult:
    ...
    For a signed audit trail of permission checks, see
    :class:`openterms.IngestClient`.
    """
    return _default_client.check(domain, action)


def discover(domain: str) -> DiscoveryResult | None:
    """
    Return the ``discovery`` block from *domain*'s openterms.json.
...
def permission_receipt(
    ...
    Returns:
        A :class:`PermissionReceipt` dict-serialisable via ``.to_dict()``.

    Example::

        result = openterms.check("github.com", "api_access")
        rec = openterms.permission_receipt(
            "github.com", "api_access", result.decision
        )
        print(rec.to_dict())

    Renamed from ``openterms.receipt()`` in 1.0.0 — see CHANGELOG.
    """
    return _default_client.permission_receipt(domain, action, decision)
